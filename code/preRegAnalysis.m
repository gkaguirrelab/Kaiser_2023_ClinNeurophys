...
% run, it will aggregate data for given subjects and blink features. It 
% will then produce a table which describes the mean R2 by puff pressure
% across subjects, the R and IQR of the test-retest correlation for slope 
% and for offset. The R2 of the habituation effect across trials, and the 
% R and IQR of the test retest of the slope of that habituation effect.
...
% choose blink features: the first set of features is for the puff pressure analyses 
% and the second set of parameters is for the habituation analyses
...
% choose subject list
subList = {15512, 15507, 15506, 14596, 14595, 14593, 14592, 14591, 14587, 14586};
...
for vv = 1:length(varNamesToPlot1)
    ...
    for ss = 1:length(subList)
        ...
        % find scans for desired subject and confirm they are valid
...
%% get r and iqr of test retest for slope and offset
...
for vv = 1:length(varNamesToPlot1)
    ...
    % calculate slope IQR
    ab = abs(pX);
    percentS = ((pY - pX) ./ ab)*100;
    MyFun = @(d) iqr(d);
    slopestat = sort(bootstrp(1000,MyFun,percentS));
    iqrslope(end+1) = iqr(percentS);
    CILS(end+1) = slopestat(25);
    CIHS(end+1) = slopestat(975);
    
    % calculate offset IQR
    ab = abs(oX);
    percentO = ((oY - oX) ./ ab)*100;
    MyFun = @(d) iqr(d);
    offsetstat = sort(bootstrp(1000,MyFun,percentO));
    iqroffset(end+1) = iqr(percentO);
    CILO(end+1) = offsetstat(25);
    CIHO(end+1) = offsetstat(975);
...
%% get r and iqr of the test retetest of habituation across trials
...
for vv = 1:length(varNamesToPlot2)
    ...
    habslope(end+1) = co(1,2);
    
    % calculate CIs and add to table arrays
    slopestat = sort(bootstrp(1000,@corr,sessOneSlopes,sessTwoSlopes));
    CILH(end+1) = slopestat(25);
    CIHH(end+1) = slopestat(975);
...
col = {'R2 puff pressure model', 'CI low 1', 'CI high 1', 'TR r slope', 'CI low 2', 'CI high 2', ...
    'IQR slope', 'CI low S', 'CI high S', 'TR r offset',  'CI low 3', 'CI high 3', 'IQR offset',  ... 
    'CI low O', 'CI high O', 'R2 habituation', 'CI low 4', 'CI high 4', ...
    'TR r habituation', 'CI low H', 'CI high H', 'habituation iqr', 'CI low 5', 'CI high 5'};
...
T = table(Rs', CIL1', CIH1', rslope', CIL2', CIH2', iqrslope', CILS', CIHS', roffset', ...
    CIL3', CIH3', iqroffset', CILO', CIHO', r2Hab', CIL4', CIH4', habslope', CILH', CIHH', ...
    rHab',  CIL5', CIH5', 'VariableNames', col, 'RowNames', row);
